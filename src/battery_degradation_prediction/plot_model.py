...
def parity_plot(test_y, predictions):
    ...
    ax.set_xlim([np.min(np.concatenate((test_y, predictions))) - 0.1, np.max(np.concatenate((test_y, predictions))) + 0.1])
    ax.set_ylim([np.min(np.concatenate((test_y, predictions))) - 0.1, np.max(np.concatenate((test_y, predictions))) + 0.1])
    ax.annotate(f"R² = {r2:.2f}", xy=(0.80, 0.05), xycoords="axes fraction")
    plt.legend()
    plt.show()


def plot_future_capacity(df_feature, input_shape, model, X_scaler, y_scaler, future_cycle, window_size):
    """TODO"""
