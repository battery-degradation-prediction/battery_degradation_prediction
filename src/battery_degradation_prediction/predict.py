...
from battery_degradation_prediction.load_data import load_unsupervised_data, load_supervised_data
...
def main():
    ...
    for future_cycle in future_cycles:
        ...
        x_windows_inv = X_scaler.inverse_transform(x_windows.reshape(-1, num_features)).reshape(x_windows_shape)
        ...
        ax.scatter(capacity_pred,
                voltage_pred,
                label='Pred', s=6, facecolors="none", edgecolors="k")
        ax.plot(capacity_true,
                voltage_true,
                '--k',
                label='GT')
