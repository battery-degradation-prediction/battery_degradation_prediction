...
def get_data(num_data):
    ...
    X = np.array([binary_to_list(bin(x)[2:], max_digit) for x in X])
...
class Net(nn.Module):
    ...
    def __init__(self, max_digit):
        ...
        self.fc1 = nn.Linear(max_digit, 1000)
...
def test(X_test, model, test_labels, classes):
    """TODO"""
    with torch.no_grad():
        model.eval()
        outputs = model(X_test)
        _, pred_tests = torch.max(outputs, 1)
        test_acc = calculate_accuracy(pred_tests, test_labels)

    for idx, (x_bin, pred_test) in enumerate(zip(X_test, pred_tests[:100])):
...
def main():
    ...
    model = train(X_train, y_train, model, epochs, optimizer, criterion)
    test_acc = test(X_test, model, test_labels, classes)
