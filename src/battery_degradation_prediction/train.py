...
def main():
    ...
    test_size = 0.1
    dev_x, dev_y, test_x, test_y, y_scaler = load_data(df_discharge, test_size, feature_names)
    ...
    pred_inv = y_scaler.inverse_transform(pred.reshape(1, -1)).reshape(-1,)
    test_inv = y_scaler.inverse_transform(test_y.reshape(1, -1)).reshape(-1,)
