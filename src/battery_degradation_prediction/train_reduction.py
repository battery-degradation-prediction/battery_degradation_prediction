...
def main():
    ...
    test_size = 0.2
    num_features = 4
    (dev_x, dev_y), (test_x, test_y), X_scaler = load_unsupervised_data(
    ...
    # draw_voltage_capacity(model, x_pred, X_scaler,
    #                       dev_y[future_cycle+1:future_cycle+2], num_features)
    ...
    for idx, future_cycle in enumerate(range(5, 131, 20)):
        ...
        if len(dev_y[future_cycle + 1 : future_cycle + 2]):
            (
                ...
                capacity_overtime,
                voltage_overtime,
            ) = get_voltage_capacity(
                model,
                x_pred,
                X_scaler,
                dev_y[future_cycle - future : future_cycle - future + 1],
                num_features,
            )

            ax.scatter(
                capacity_overtime_pred[0],
                voltage_overtime_pred[0],
                marker=markers[idx],
                label=f"Pred (cycle={future_cycle})",
                s=25,
            )  # , facecolors="none", edgecolors="k")
            ax.plot(capacity_overtime[0], voltage_overtime[0], "--k")
            # label='GT')
        else:
            (
                capacity_overtime_pred,
                ...
                _,
                _,
            ) = get_voltage_capacity(model, x_pred, X_scaler, dev_y[:1], num_features)
            ax.scatter(
                capacity_overtime_pred[0],
                voltage_overtime_pred[0],
                label=f"Pred (cycle={future_cycle})",
...
def draw_voltage_capacity(model, X, X_scaler, y, num_features):
    ...
    for cycle_num in range(1):
        ax.scatter(
            capacity_overtime_pred[cycle_num],
